...
def comp_split(dataset, base="splits"):
    if not (os.path.exists(base+"/"+"comp_test_ids.txt")
            ):
        raise Exception("There is an error and the dataset reader cannot find the "
                        "comp_test_ids.txt file. Please make sure your python paths "
                        "are configured correctly")

    comp_ids = read_ids("comp_test_ids.txt",base)

    #return the stances that meet these criteria
    comp_stances = []

    for stance in dataset.stances:
        if stance['Body ID'] in comp_ids:
            comp_stances.append(stance)
